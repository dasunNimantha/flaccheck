...
use lossless_scan_testkit::{core_corpus, evaluate_suite, full_corpus, GroundTruth, SuiteMetrics};
...
fn category_genuine_wideband_never_transcoded() {
    ...
    for case in corpus {
        ...
        if !lossless_scan_testkit::matches_truth(&r, GroundTruth::Genuine) {
            fails += 1;
        }
    }
    assert_eq!(
...
fn category_brick_wall_detected() {
    ...
    for case in corpus {
        ...
        total += 1;
        let r = analyze_pcm(case.id, &case.pcm, &config).unwrap();
        if lossless_scan_testkit::matches_truth(&r, GroundTruth::Transcoded) {
            hits += 1;
        }
    }
...
fn category_joint_stereo_detected() {
    ...
    for case in corpus {
        ...
        total += 1;
        let r = analyze_pcm(case.id, &case.pcm, &config).unwrap();
        if lossless_scan_testkit::matches_truth(&r, GroundTruth::NotGenuine) {
            hits += 1;
        }
    }
    assert!(total > 0);
...
fn category_inconclusive_tones() {
    ...
    for case in corpus {
        ...
        total += 1;
        let r = analyze_pcm(case.id, &case.pcm, &config).unwrap();
        if lossless_scan_testkit::matches_truth(&r, GroundTruth::Inconclusive) {
            hits += 1;
        }
    }
    let rate = hits as f64 / total as f64;
...
fn category_hires_fakes_not_genuine() {
    ...
    for case in corpus {
        ...
        let r = analyze_pcm(case.id, &case.pcm, &config).unwrap();
        assert!(
            lossless_scan_testkit::matches_truth(&r, GroundTruth::NotGenuine),
