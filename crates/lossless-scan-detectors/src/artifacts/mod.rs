...
pub fn analyze(pcm: &PcmBuffer, light_only: bool, thresholds: &Thresholds) -> ArtifactResult {
    let mut evidence = Vec::new();
    let mut suspicion = 0.0f64;

    let noise_score = noise_floor_shape(pcm, thresholds);
    ...
    if !light_only {
        ...
        if pcm.channels >= 2 {
            let joint = joint_stereo_score(pcm, thresholds);
            evidence.push(Evidence::new(
                "artifacts",
                "joint_stereo",
                joint,
                thresholds.weight_joint_stereo,
...
fn noise_floor_shape(pcm: &PcmBuffer, thresholds: &Thresholds) -> f64 {
    let mono = pcm.left();
    ...
    if flatness > thresholds.noise_floor_flatness_high && mean < 1e-8 {
        0.7
    } else if flatness > thresholds.noise_floor_flatness_mid && mean < 1e-6 {
        0.4
    } else {
        0.0
    }
...
fn pre_echo_score(pcm: &PcmBuffer) -> f64 {
    ...
    let frame = (pcm.sample_rate as f64 * 0.023) as usize;
    if env.len() < frame * 4 {
        return 0.0;
    }
    let mut hits = 0usize;
    ...
    for i in (frame * 2..env.len().saturating_sub(frame)).step_by(frame) {
        let pre: f64 = env[i.saturating_sub(frame / 4)..i]
            ...
            / (frame / 4) as f64;
        let post: f64 =
            env[i..i + frame / 4].iter().map(|&e| e as f64).sum::<f64>() / (frame / 4) as f64;
...
fn phase_discontinuity_score(pcm: &PcmBuffer) -> f64 {
    ...
    let rate = jumps as f64 / tests as f64;
    // Random noise pegs jump rate at ~1.0; real codec frames are sparser.
    if rate > 0.85 {
        return 0.0;
    }
    (rate * 2.5).clamp(0.0, 1.0)
}

fn joint_stereo_score(pcm: &PcmBuffer, thresholds: &Thresholds) -> f64 {
    let l = pcm.left();
    let r = pcm.right();
    ...
    for i in (0..n).step_by(step) {
        mids.push((l[i] + r[i]) as f64 * 0.5);
        sides.push((l[i] - r[i]) as f64 * 0.5);
    }
    ...
    if mid_var > 1e-10 && side_var / mid_var < thresholds.joint_stereo_ratio_high {
        thresholds.joint_stereo_score_high
    } else if mid_var > 1e-10 && side_var / mid_var < thresholds.joint_stereo_ratio_mid {
        thresholds.joint_stereo_score_mid
    } else {
        0.0
    }
}
...
mod tests {
    ...
    fn artifacts_run_on_stereo() {
        ...
        let r = analyze(&pcm, false, &Thresholds::default());
