...
pub fn fuse_evidence(evidence: &[Evidence]) -> (TranscodeVerdict, f64, Option<String>, Option<u32>) {
  ...
  // Band-limited abstention (Tier 5)
  ...
  for e in evidence {
    if e.detector == "hires" || e.signal == "spectral_info_score" || e.signal == "abstain_band_limited" {
      continue;
    }
    // Informational signals carry physical units, not scores
    if matches!(
      e.signal.as_str(),
      "cutoff_hz" | "rolloff_hz" | "codec_guess" | "est_bitrate_kbps" | "mp3_pqmf" | "ml_abstain" | "ml_noop"
    ) || e.weight <= 0.0 {
      continue;
    }
    if e.value > 1.0 {
  ...
  let confidence = match verdict {
    ...
    TranscodeVerdict::Genuine => 1.0 - normalized,
    TranscodeVerdict::Inconclusive => 0.0,
  };

  (verdict, confidence.clamp(0.0, 1.0), codec_guess, est_bitrate)
}
