...
pub struct Thresholds {
    ...
    /// AAC encoder pre-cutoff shelf score at/above which, combined with a sub-Nyquist
    /// cutoff, is treated as an AAC transcode fingerprint (AAC cliffs are often gentler
    /// than MP3 and may not reach `brick_wall_transcoded_min` alone).
    pub aac_shelf_transcoded_min: f64,
    /// Cutoff (Hz) above which we do not treat AAC shelf as a transcode signal — high-bitrate
    /// AAC at or near Nyquist is indistinguishable from genuine on spectral grounds alone.
    pub aac_cutoff_max_hz: f64,
...
impl Default for Thresholds {
    fn default() -> Self {
        Self {
            ...
            aac_shelf_transcoded_min: 0.65,
            aac_cutoff_max_hz: 22000.0,
