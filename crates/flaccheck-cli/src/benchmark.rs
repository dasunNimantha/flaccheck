        ...
        return Err(format!(
            "{} is a directory, not a JSON manifest.\n\n\
             ...
             Benchmark compares results against a labeled dataset manifest, e.g.:\n\
               flaccheck benchmark datasets/output/synthetic/manifest.json",
            path.display(),
            path.display()
        ));
    }
    ...
    for entry in &entries {
        *per_label.entry(entry.label.clone()).or_insert(0) += 1;
        ...
        match (result.transcode_verdict, actual_transcode) {
            (flaccheck_core::TranscodeVerdict::Inconclusive, _) => inconclusive += 1,
            (
                flaccheck_core::TranscodeVerdict::Transcoded
                | flaccheck_core::TranscodeVerdict::Suspicious,
                true,
            ) => tp += 1,
            (
                flaccheck_core::TranscodeVerdict::Transcoded
            ...
            (flaccheck_core::TranscodeVerdict::Genuine, true) => fn_ += 1,
            (flaccheck_core::TranscodeVerdict::Genuine, false) => tn += 1,
        }
    }
...
mod tests {
    ...
    fn rejects_directory_as_manifest() {
        ...
        assert!(err.contains("flaccheck scan"));
    }
