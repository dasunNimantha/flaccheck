...
fn match_codec_signature(
    ...
    if brick_strength < 0.25
        && steepness < thresholds.steepness_codec_db_per_oct * 0.8
        && shelf_score < thresholds.aac_shelf_transcoded_min
    {
    ...
    let brick_for_codec = if shelf_score >= thresholds.aac_shelf_transcoded_min {
        brick_strength.max(0.15)
    } else {
        brick_strength
    };
    ...
    let mp3_score = best_mp3.2 * brick_for_codec.max(0.4);
    let aac_score = best_aac.2 * (brick_for_codec.max(0.15) + shelf_score * 0.55).min(1.0);
    let opus_score = best_opus.2 * brick_for_codec.max(0.5);

    // When the AAC shelf is present, prefer AAC over MP3 even if cutoff distances are similar.
    let aac_bias = if shelf_score >= thresholds.aac_shelf_transcoded_min {
        1.15
    } else {
        1.0
    };
    let aac_score = aac_score * aac_bias;
...
mod tests {
    use super::*;
    use flaccheck_core::{PcmBuffer, Thresholds};
