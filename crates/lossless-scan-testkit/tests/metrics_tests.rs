...
use lossless_scan_core::{AnalysisResult, TranscodeVerdict};
use lossless_scan_testkit::{evaluate_suite, full_corpus, matches_truth, GroundTruth, LabeledCase};

fn stub_result(verdict: TranscodeVerdict, confidence: f64, info: f64) -> AnalysisResult {
    AnalysisResult {
        path: "test".into(),
        transcode_verdict: verdict,
        hires_verdict: lossless_scan_core::HiresVerdict::Unknown,
        confidence,
        spectral_info_score: info,
        codec_guess: None,
        est_source_bitrate_kbps: None,
        evidence: vec![],
        mode: lossless_scan_core::ScanMode::Balanced,
        duration_secs: 3.0,
        sample_rate: 44100,
        channels: 1,
